# The Brain — spine architecture (2026-09-09)

## Product

One app: **Hybrid Athlete** — central hub, connectors, AI coach chat.

- Shared **Home** for Strength / Engine / Nutrition (room accent switch)
- **Connectors:** WHOOP (Brain owner proxy), Concept2 proxies (wired, UI later)
- **Coach:** `brain-coach` Netlify function → OpenRouter on Brain owner site
...
| Path | Role |
| --- | --- |
| `packages/brain/` | **The Brain** — pure hub logic (packet, readiness, coach context) |
| `apps/athlete/` | Athlete UI + Netlify deploy root |
| `apps/mobile/capacitor/` | Android shell → `webDir: ../../athlete` |
