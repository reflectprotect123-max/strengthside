...
> **AUTHORITATIVE CHECKPOINT — 8 September 2026 (HPP no Engine block).**
...
| **Cache** | **`the-hybrid-athlete-blank-v204`** (`LOCAL_BUILD` + SW `CACHE` must match) |
| **Capgo** | **`dogfood` + `live` @ `1.0.69`** (HPP days are lifts + Recovery Breathing notes only)
...
**Phone:** Settings → Check for updates → expect **1.0.69** (Settings → Check for updates)
...
| Channels | **`dogfood` + `live`** both @ **`1.0.69`** (HPP no leaked Engine block) |
...
| Capgo | **1.0.69** on `dogfood` + `live` (blank-v204; HPP Recovery Breathing stays a text note) |
| Cache | `the-hybrid-athlete-blank-v204` |
