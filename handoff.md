...
| **Open PR** | **None** |
| **Cache** | **`the-hybrid-athlete-engine-v117`** (`LOCAL_BUILD` + SW `CACHE` together) |
| **Capgo** | **`dogfood` @ `1.0.23`** (uploaded 31 Aug — boot-fix + Track Dawn polish, cache v117) |
...
| Channel / bundle | `dogfood` / **`1.0.23`** |
| Upload | `CAPGO_BUNDLE_VERSION=1.0.24 bash apps/mobile/capacitor/scripts/upload-capgo-bundle.sh` |
...
| # | Item | Status | Notes |
| --- | --- | --- | --- |
| 1 | Upload Capgo bundle (v99 cache) | **done** | **1.0.23** on dogfood (31 Aug, cache v117 — template UX + Track Dawn polish). Phone: Settings → Check for updates. |
