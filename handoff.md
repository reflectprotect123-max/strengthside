...
| **Cache** | **`the-hybrid-athlete-blank-v213`** (`LOCAL_BUILD` + SW `CACHE` must match) |
| **Capgo** | **`dogfood` + `live` @ `1.0.78`** (Warm-up + Cool-down boxes on strength and Engine) |
...
**Phone:** Home/Settings show an **App update** banner when Capgo has a newer bundle ready. Settings → Look for app update. Expect **1.0.78**.
...
| Channels | **`dogfood` + `live`** both @ **`1.0.78`** (Warm-up + Cool-down boxes) |
