...
| **Capgo** | **`dogfood` + `live` @ `1.0.60`** (cache blank-v179) |
...
**Phone:** Settings → Check for updates → expect **1.0.60** (cache **blank-v179**).
...
| Channels | **`dogfood` + `live`** both @ **`1.0.60`** |
| Upload dogfood | `CAPGO_CHANNEL=dogfood CAPGO_BUNDLE_VERSION=1.0.60 bash apps/mobile/capacitor/scripts/upload-capgo-bundle.sh` |
| Ship dogfood + live | `CAPGO_BUNDLE_VERSION=1.0.60 bash apps/mobile/capacitor/scripts/ship-capgo.sh` (fails hard without token) |
| Point live | from `apps/mobile/capacitor`: `npx @capgo/cli@latest channel set live com.hybrid.athlete --apikey "$CAPGO_TOKEN" --bundle 1.0.60` |
