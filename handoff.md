...
| **Cache** | **`the-hybrid-athlete-blank-v211`** (`LOCAL_BUILD` + SW `CACHE` must match) |
| **Capgo** | **`dogfood` + `live` @ `1.0.76`** (empty catalog + unseeded HPP days) |
...
**Phone:** Home/Settings show an **App update** banner when Capgo has a newer bundle ready. Settings → Look for app update. Expect **1.0.76**.
...
| Channels | **`dogfood` + `live`** both @ **`1.0.76`** (empty catalog + unseeded days) |
...
| Capgo | **1.0.76** on `dogfood` + `live` (blank-v211 empty catalog + unseeded HPP days) |
| Cache | `the-hybrid-athlete-blank-v211` |
