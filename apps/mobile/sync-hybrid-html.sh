...
for f in adaptive-bundle.js session-chrome.js session-flow.js rest-overlay.js work-overlay.js log-columns.js strength-one-set-logger.js exercise-load-profiles.js exercise-search-index.js exercise-search.js exercise-history-seed.js exercise-history-seed-apply.js; do
  if [[ -f "$SRC_DIR/$f" ]]; then
    cp "$SRC_DIR/$f" "$ROOT/$f"
